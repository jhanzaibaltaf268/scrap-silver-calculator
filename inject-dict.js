...
for (const file of files) {
  if (!file.endsWith('.html') || file === 'index.html' || file === 'calculadora-de-plata-de-desecho.html') {
    // wait, we ALSO want to update index.html and silver-scrap-calculator.html so their navigation works locally too 
  }
  
  const filePath = path.join(esDir, file);
  let content = fs.readFileSync(filePath, 'utf8');
  
  // They either have: window.MenuTranslations = Object.assign({ slugs: {"silver-scrap-calculator.html"...} }, {});
