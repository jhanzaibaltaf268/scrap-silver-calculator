...
// --- CLEAN CONTENT BLOCKS (Pure UTF-8 Literals) ---
const CLEAN_BLOCKS = {
  hi: {
...
const LABELS = {
  ar: { Home: "الرئيسية", Calculators: "الحاسبات", Purity: "النقاء", Pricing: "الأأسعار", Tools: "الأدوات", Guides: "الأدلة" },
...
LANGS.forEach(lang => {
  ...
  files.forEach(file => {
    ...
    // 1. RECOVERY: SANITIZE STEPS SECTION (Aggressive wiping)
    ...
    // 2. RECOVERY: SANITIZE UNDERSTAND SECTION
  ...
  console.log(`V12 recovery successful for ${lang} folder (${files.length} files)`);
});
