...
const ES_LABELS = {
  ...
  "Free, accurate silver calculators using live spot prices. Calculate the melt value of your silver scrap, coins, jewelry, and bars instantly.": "Calculadoras de plata gratuitas usando precios spot en vivo.",
  "copied": "Copiado",
...
const FR_LABELS = {
  ...
  "Scrap Silver Calculator": "Calculateur de Ferraille d'Argent",
  "All rights reserved.": "Tous droits r\u00e9serv\u00e9s.",
  "Prices are for informational purposes only.": "Les prix sont \u00e0 titre informatif.",
  "Free, accurate silver calculators using live spot prices. Calculate the melt value of your silver scrap, coins, jewelry, and bars instantly.": "Calculateurs gratuits utilisant les prix en direct.",
  "copied": "Copi\u00e9",
...
const L_DICTS = {
  ...
  fr: Object.assign({ slugs: FR_SLUGS }, FR_LABELS)
};
...
LANGS.forEach(lang => {
    ...
    files.forEach(file => {
        ...
        // 1. CLEANUP OLD SCRIPTS (Regex to find any window.MenuTranslations block)
        content = content.replace(/<script>\s*window\.MenuTranslations\s*=\s*\{.*?\};\s*<\/script>/gs, '');
        content = content.replace(/<script>\s*window\.MenuTranslations\s*=\s*\{.*?\}\s*<\/script>/gs, '');

        // 2. INJECT NEW SCRIPT AT THE VERY TOP OF HEAD
    ...
    console.log(`Sync V6 (Final) completed for /${lang}/ (${count} files).`);
});

console.log('ALL FIXES APPLIED (SYNC V6).');
