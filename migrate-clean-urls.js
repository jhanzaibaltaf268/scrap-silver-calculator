...
function processFile(filePath) {
    ...
    // 1. Fix index in Logo/Home links (if missing from components.js injection)
    // Actually these are injected usually, but some pages might have hardcoded ones.
    content = content.replace(/href="index\/"/g, 'href="/"');

    // 2. Fix localized index references in subdirectories
    content = content.replace(/href="\.\.\/index\/"/g, 'href="/"');

    // 3. Fix breadcrumbs: [{label:'Home',href:'/'}] -> [{label:'Home',href:'/'}]
    ...
    if (slugsMatch) {
        ...
        // Example: "silver-scrap-calculator":"calculadora-de-plata-de-desecho"
    ...
    // 5. Global cleanup for any href="/something/" -> href="/something/"
    // Note: We add a trailing slash because vercel.json has trailingSlash: true
    content = content.replace(/href="\/([^"]+?)\.html"/g, 'href="/$1/"');

    // 6. Fix relative href="something/" -> href="something/"
