...
async function processFile(filename) {
  ...
  $('script').each((i, el) => {
    ...
    if (code && code.includes('SiteComponents.renderBreadcrumb')) {
      ...
      code = code.replace(/{label:\s*'Home'/g, `{label: window.MenuTranslations['Home'] || 'Startseite'`);
      $(el).html(code);
    }
  });
  ...
  const langScript = `\n<script>\n  window.MenuTranslations = ${JSON.stringify(require('./js/generate-languages').de || {})}; \n</script>\n`;
...
async function run() {
  ...
  console.log('All files translated directly to German!');
}
